...
type Observer interface {
	...
	ReceivedSufficientNilPrevotes(messages Messages, threshold int)
}
...
func (p *Process) handlePrevote(prevote *Prevote) {
	...
	if n := p.state.Prevotes.QueryByHeightRoundBlockHash(p.state.CurrentHeight, p.state.CurrentRound, block.InvalidHash); n > 2*p.state.Prevotes.F() {
		...
		p.observer.ReceivedSufficientNilPrevotes(p.state.Prevotes.QueryMessagesByHeightRound(p.state.CurrentHeight, p.state.CurrentRound), 2*p.state.Prevotes.F()+1)
	}
