...
var _ = Describe("Validator", func() {
	...
	Context("when an invalid commit is provided", func() {
		It("should return false", func() {
			...
			commit := block.Commit{
				Polka:       precommit1.Polka,
				Signatures:  sig.Signatures{precommit1.Signature, testutils.RandomSignature()},
				Signatories: sig.Signatories{precommit1.Signatory, testutils.RandomSignatory()},
			}
			Expect(validator.ValidateCommit(commit)).To(BeFalse())

			newBlock := testutils.SignBlock(block.Block{Time: time.Now(), Height: 2, ParentHeader: parent.Header}, signer1)
			newPropose := testutils.GenerateSignedPropose(*newBlock, 0, signer1)
			newPropose.LastCommit = &commit
			Expect(validator.ValidatePropose(newPropose)).To(BeFalse())
		})
	})
})
